...
import cn.com.kind.im.helper.SocketClientState;
import cn.com.kind.im.helper.SocketConfigure;
import cn.com.kind.im.helper.SocketHeartBeatHelper;
import cn.com.kind.im.helper.SocketInputReader;
import cn.com.kind.im.helper.SocketPacket;
import cn.com.kind.im.helper.SocketPacketHelper;
...
public class SocketClient {
    ...
    public void connect() {
        ...
        getSocketConfigure().setCharsetName(getCharsetName()).setAddress(getAddress()).setSocketPacketHelper(getSocketPacketHelper());
    ...
    public void sendHeartPackage(){
        getSocketConfigure().setHeartBeatHelper(getHeartBeatHelper());
    ...
    /**
     * 注册监听回调
     * @param SocketState 回调判断是否断开连接
     */
    public SocketClient registerSocketClientState(SocketClientState SocketState) {
        if (!getSocketClientStates().contains(SocketState)) {
            getSocketClientStates().add(SocketState);
        }
        return this;
    }
    ...
    private ArrayList<SocketClientState> socketClientStates;
    protected ArrayList<SocketClientState> getSocketClientStates() {
        if (this.socketClientStates == null) {
            this.socketClientStates = new ArrayList<SocketClientState>();
        }
        return this.socketClientStates;
    }
    ...
    private void Disconnected() {
        ...
        ArrayList<SocketClientState> delegatesCopy =
                (ArrayList<SocketClientState>) getSocketClientStates().clone();
        int count = delegatesCopy.size();
        for (int i = 0; i < count; ++i) {
            delegatesCopy.get(i).onDisconnected(this);
        }
        ...
        public void run() {
            ...
            try {
                while (self.isConnected()
                    ...
                    SocketInputReader reader= self.getSocketInputReader();
                     if(reader.ready()){
                         self.__i__onReceivePacketBegin(packet);
