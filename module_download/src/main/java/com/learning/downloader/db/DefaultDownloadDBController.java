...
import static com.learning.downloader.domain.DownloadInfo.STATUS_ERROR;
import static com.learning.downloader.domain.DownloadInfo.STATUS_PAUSED;
...
public final class DefaultDownloadDBController implements DownloadDBController {
  ...
  public static final String SQL_UPDATE_DOWNLOAD_INFO = String.format(
      "REPLACE INTO %s (_id,supportRanges,createAt,uri,path,size,progress,status,fileid) VALUES(?,?,?,?,?,?,?,?,?);",
  ...
  @Override
  public List<DownloadInfo> findAllDownloadFailure() {
    Cursor cursor = readableDatabase.query(DefaultDownloadHelper.TABLE_NAME_DOWNLOAD_INFO,
            DOWNLOAD_INFO_COLUMNS, "status=?", new String[]{
                    String.valueOf(STATUS_ERROR)}, null, null, "createAt desc");
  ...
  private void inflateDownloadInfo(Cursor cursor, DownloadInfo downloadInfo) {
    ...
    downloadInfo.setFileid(cursor.getString(8));
  }
  ...
  public void createOrUpdate(DownloadInfo downloadInfo) {
    writableDatabase.execSQL(
        ...
        new Object[]{
            ...
            downloadInfo.getSize(), downloadInfo.getProgress(), downloadInfo.getStatus(),downloadInfo.getFileid()});
  }
